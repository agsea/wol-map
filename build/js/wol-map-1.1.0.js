/*! WolMao v1.1.0 | (c) aegean | Created on 2017/3/7 */
/*! 基于Openlayers的地图常用功能封装，实现地图的简单快速开发，依赖Openlayers v4.1.1版本 /*
/*! Modified on 2017/05/30 13:32:02 */
...
(function(global) {
    ...
    wol.Locator = function(mapViewer) {
        ...
        function _init() {
            //检查locateLayer是否存在
            var tempLayer = mapViewer.getLayerByName('locateLayer');
            if(tempLayer) {
                mapViewer.removeLayer(tempLayer);
            }
     ...
     * @param {number} times - 动画次数，可选参数，默认为6
     */
    wol.Locator.prototype.locate = function(feature, duration, times) {
        //是否正在定位
        ...
        this.locateProxy(feature, duration, times);
    };
     ...
     * @param {number} times - 动画次数，可选参数，默认为6
     */
    wol.Locator.prototype.locateProxy = function(feature, duration, times) {
        this.panTo(feature, 600);
    ...
    wol.Locator.prototype.locatePoint = function(feature, duration) {
        ...
        function animate(event) {
            ...
            var style = self.createPointFlashStyle(elapsedRatio, 5, 35);
        ...
        listenerKey = self.getMap().on('postcompose', animate);
        //立即触发动画
        self.getMap().render();
    };
    ...
    wol.Locator.prototype.locateLineString = function(feature, duration) {
        ...
        listenerKey = this.getMap().on('postcompose', animate);
        self.getMap().render();
    };
     ...
     * @param {number} duration - 动画持续时间，可选参数，默认为0
     */
    wol.Locator.prototype.panTo = function(target, duration) {
        var self = this;
        ...
        if(duration == undefined || duration == 0) {
            ...
            view.animate({center: coord}, {duration: duration});
        }
    }
})(window);;(function(global) {
    global.wol = global.wol || {};
    ...
    var _iconPath = newPath + '/image/tracer.png';
    ...
    wol.Tracer = function(mapViewer) {
        var _map = mapViewer.getMap();
        var _traceLayer, _traceLine, _flashLine, _flashPoint;
        //默认配置项
        var _defaultOption = {
            source: null,
            speed: 3,
        ...
        this.init = function(option) {
            ...
            //检查traceLayer是否存在
            var tempLayer = mapViewer.getLayerByName('traceLayer');
            if(tempLayer) {
                mapViewer.removeLayer(tempLayer);
            }
            _traceLayer = wol.util.createVectorLayer({
                ...
                features: [_traceLine, _flashLine, _flashPoint],
                updateWhileAnimating: true,
            ...
            if(option.ifLocate) {
                this.panTo(this.animateParam.tempCoord, 400);
            }
        }
    ...
    wol.Tracer.prototype.play = function() {
        ...
        if(!self.status.initialize) {
            ...
            if(!self.animateParam.ifDynamicTrack) {
                _switchView(self.getMap(), self.animateParam.tempCoord);
                delay = 100;
            }else {
                self.getMap().getView().setZoom(16);
            }
        ...
        setTimeout(function() {
            ...
            self.timer = setInterval(function() {
                if(self.animateParam.index < self.animateParam.coordLen - 1) {
                    ...
                    //是否动态追踪
                    if(self.animateParam.ifDynamicTrack) {
                        self.panTo(self.animateParam.tempCoord, 100);
                    }else {
                        ifContain = _ifContainFlashPoint(self.getMap(), self.animateParam.tempCoord);
                        if(!ifContain) {
                            _switchView(self.getMap(), self.animateParam.tempCoord);
                        }
                    }
     ...
     * @param {object} option.anchor - 图标定位数组，默认为 [0.5, 1]
     * @param {object} option.anchorXUnits - X方向定位所用单位，可选值为'fraction'、'pixels'，默认为'fraction'；设置为'fraction'时，option.anchor应采用0~1之间的数值
     * @param {object} option.anchorYUnits - Y方向定位所用单位，可选值为'fraction'、'pixels'，默认为'fraction'；设置为'fraction'时，option.anchor应采用0~1之间的数值
     * @param {object} option.src - 图标文件的url
     * @param {object} option.scale - 图标缩放比例，默认为1
     * @param {object} option.opacity - 图标透明度，默认为1
     */
    wol.Tracer.prototype.setIcon = function(option) {
        var self = this;
        var iconStyle = _getIconStyle(option);
        self.getFlashPoint().setStyle(iconStyle);
    }
     ...
     */
    function _switchView(map, coord) {
        var view = map.getView();
        view.setCenter(coord);
        view.setZoom(17);
    }
    ...
    function _getInterpolation(point1, point2, step) {
        ...
        var step = step, targetArray = [point1];
        var tempX, tempY;
     ...
     * @param {ol.proj.ProjectionLike} from - 待转换投影坐标系
     * @param {ol.proj.ProjectionLike} to - 目标投影坐标系
     */
     ...
     */
    wol.util.isExtent = function(arg0) {
        if((arg0 instanceof Array) && arg0.length == 4) {
            return true;
        }else {
            return false;
        }
     ...
     * @param {Array} option.layers - 图层配置，通过传入一至多个配置项创建图层，每个配置项须包含图层类型，图层名称，地图服务地址等必要参数；图层类型可选值为'XYZ'、'Mask'、'Vector'、'WMS'，
     * 图层类型为'XYZ'或'WMS'时需指定地图服务地址；配置项中其他参数设置请参见{@link wol.util.createVectorLayer}、{@link wol.util.createXYZLayer}、{@link wol.util.createWMSLayer}、
     * {@link wol.util.createMaskLayer}
    ...
    wol.MapViewer.prototype.extentAt = function(target, duration) {
        ...
        if(target instanceof ol.geom.Geometry) {
            target = target.getExtent();
        }else if(!wol.util.isExtent(target)) {
            console.error('The type of parameter "extent" is not correct, it must be ol.Extent or ol.geom.Geometry.');
            return;
        }
        var extent = target;
        ...
        var view = this.getMap().getView();
        //var center = view.getCenter();
        var tarCenter = ol.extent.getCenter(extent);
        //是否开启动画
        if(duration == undefined || duration == 0) {
            view.setCenter(tarCenter);
            view.fit(extent, ol.extent.getSize(extent));
        }else {
            /*var pan = ol.animation.pan({
                source: center,
                duration: duration
            ...
            view.animate({zoom: 17}, {center: tarCenter}, {duration: duration});
        }
    }
...
//# sourceMappingURL=wol-map-1.1.0.js.map